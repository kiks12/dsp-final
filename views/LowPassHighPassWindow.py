...
class lowPassHighPassWindow(ctk.CTk):
    ...
    def initialize_filter_form(self):
        self.filter_form_frame = ctk.CTkFrame(
            self.filter_tab, width=250, height=600)
        self.create_filter_form()

        self.filter_plot_frame = ctk.CTkFrame(
            self.filter_tab, width=750, height=600)
        ctk.CTkLabel(self.filter_plot_frame, text="Please Load Audio first.").place(
            relx=0.5, rely=0.5, anchor="center")
    ...
    def create_filter_form(self):
        ctk.CTkLabel(self.filter_form_frame, text="Cut Off Frequency (Hz)", font=(
            "Helvetica", 16)).pack(pady=10)
        self.cut_off_slider = ctk.CTkSlider(
            self.filter_form_frame, from_=1, to=22000, variable=self.cut_off, command=self.update_cut_off_slider)
        ...
        ctk.CTkButton(self.filter_form_frame, text="Low Pass Filter",
                      command=lambda: self.apply_filter('lowpass')).pack(pady=10)
        ctk.CTkButton(self.filter_form_frame, text="High Pass Filter",
    ...
    def plot_audio_signal(self, original_signal, filtered_signal, plot_frame):
        ...
        if filtered_signal is not None:
            ...
            ax.plot(filtered_signal, label=filter_label,
        ...
        ax.set_xlim(-0.02 * len(original_signal),
                    len(original_signal) + 0.02 * len(original_signal))
    ...
    def apply_filter(self, filter_type):
        ...
        if filter_type == 'lowpass':
            ...
            self.filtered_signal = self.controller.highpass_filter(
                self.audio_signal, self.sampling_rate, cutoff=cutoff_frequency)
    ...
    def save_filtered_audio(self):
        if self.filtered_signal is not None:
            ...
            if save_path:
                wav.write(save_path, self.sampling_rate,
                          self.filtered_signal.astype(np.int16))
