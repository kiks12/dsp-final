...
class MovingAverageWindow(customtkinter.CTk):
    ...
    def create_time_vector_form(self):
        ...
        self.random_apply_filter_button = customtkinter.CTkButton(
            self.random_form_frame, text="Apply Filter", width=self.RANDOM_FORM_WIDTH, height=40, corner_radius=10, command=lambda: self.controller.apply_moving_average_filter(self.random_window.get(), self.random_plot_frame)
        ).pack(pady=10)
    ...
    def create_jump_form(self):
        ...
        self.jump_slider = customtkinter.CTkSlider(
            self.jump_form, from_=0, to=1000, state="normal", variable=self.time_vector_jump, width=self.RANDOM_FORM_WIDTH, command=lambda x: self.jump_label.configure(text=f"Lines: {x}")
        )
    ...
    def create_window_form(self):
        ...
        self.window_slider = customtkinter.CTkSlider(
            self.window_form, from_=0, to=100, state="normal", variable=self.random_window, width=self.RANDOM_FORM_WIDTH, command=lambda x: self.window_label.configure(text=f"Window: {x}")
        )
