...
class AnalogToDigitalController:
    ...
    def adc_process(self, analog_signal, sampling_interval):
        if sampling_interval <= 0:
            raise ValueError(
                "Sampling interval must be a positive value greater than zero.")

        # Sampling
        sample_points = np.arange(
            0, len(analog_signal), int(sampling_interval))
        ...
        quantized_signal = np.round((sampled_signal - min_amplitude) / (
            max_amplitude - min_amplitude) * (quantization_levels - 1))
        quantized_signal = quantized_signal / \
            (quantization_levels - 1) * \
            (max_amplitude - min_amplitude) + min_amplitude
        # returning the digitized signal
        return quantized_signal, sample_points
    # plots the signal overlay
    ...
    def plot_signal(self, signal, sample_points=None, title='Signal'):
        ...
        if sample_points is not None:
            plt.plot(sample_points,
                     signal[sample_points], 'ro', label='Sampled Points')
